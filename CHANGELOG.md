...
- **All-black burst/screenshot frames from a cold VNC server** — a TightVNC-class server answers a new or long-idle client with a blank frame until its desktop poll cycle catches up; `vnc_screenshot` rode out the cold period through its long multi-request exchange but the burst's single quick baseline did not, so bursts could return all-black frames for a live desktop. New `Client.syncSettledFrame()` re-syncs (3 retries, 300 ms settle) whenever the baseline decodes all-black, used by both the burst baseline and `screenshot()`; if the frame is still black the burst response carries a WARNING line instead of presenting it as truth.
...
### Fixed
- **Agent DXGI screenshot re-enabled** — `vnc_screenshot` and the `vnc://screenshot` resource again prefer the WinMCP agent's DXGI capture (real pixels verified on a healthy console session; the earlier black frames were session-state-dependent, not a code defect). New guard: a suspiciously small JPEG payload (<32KB decoded ≈ all-black frame) is rejected and the call falls back to the RFB framebuffer, so a black frame can never be returned. Agent-path responses are tagged "(WinMCP agent DXGI capture)".
